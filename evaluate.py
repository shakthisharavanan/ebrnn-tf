...
class cnn_lstm(object):
	def __init__(self, image_size, batch_size, image_mean, checkpoints_dir, n_frames, n_hidden, n_classes):
		...
		with g2.as_default():
			...
			self.lstm_weights = tf.trainable_variables()
...
if __name__ == "__main__":
	...
	extracted_heatmaps_dir = "/mnt/workspace/ebrnn-tf/heatmaps/"
	...
	for label in labels[9:]:
		videos = sorted(glob.glob(dataset_dir + label + "/*"))
		with tqdm(total = len(videos)) as pbar:
			pbar.set_description(label)
			for video_path in videos:
				pbar.update(1)

				if not(os.path.exists(extracted_heatmaps_dir + label)):
					os.mkdir(extracted_heatmaps_dir + label)
				filename = extracted_heatmaps_dir + label + "/" + video_path.split('/')[-1].split('.')[0]
				...
				# pdb.set_trace()
				# """ For LSTM out layer """
				# P{'lstm_out'} = np.copy(p) # 24,1
				# cP{'lstm_out'} = np.copy(p)
				# lstm_weights_val = model.lstm_sess.run(model.lstm_weights)
				# # Get lstm_out to out weights
				# out_weights = lstm_weights_val[-2] # 1204, 24
				# # Get lstm_out activations
				...
				for i in range(30):
					...
					max_points.append([xmax, ymax])
				max_points = np.array(max_points)
				np.save(filename, max_points)
				# for i in range(30):
